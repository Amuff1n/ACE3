...
if (GVAR(simulateBloodRegen) == 0 || {!(isPlayer _unit) && GVAR(simulateBloodRegen) == 1}) exitWith {0};
if (GVAR(requireNoBleeding) && {IS_BLEEDING(_unit)}) exitWith {0};
...
_bloodVolumeChange = (_deltaT * EGVAR(medical_vitals,bloodRegenRate) * BLOOD_REGEN_PER_SECOND) / 1000;
